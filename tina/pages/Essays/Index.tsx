...
export default function EssaysPage(props: Props) {
  ...
  const essaysByDisplayType = props.essays.reduce((acc, item) => {
  ...
  return (
    <PageWrapper>
      ...
      <div className="mt-10 flex flex-col gap-y-10">
        {Object.keys(essaysByDisplayType).map((type) => (
          <div>
            <h2 className="text-2xl">{type}</h2>
            <ul className="flex flex-col -mx-6 sm:mx-0 divide-y w-fit max-w-2xl">
              {essaysByDisplayType[type].sort((a, b) => b.year - a.year).map((publication) => (
